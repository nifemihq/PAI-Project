...
# # Stop-level
# stop_df = pd.read_sql(
#     f"SELECT stop_id, AVG(delay_seconds) as mean_delay, COUNT(*) as n FROM delay_observations WHERE is_valid=1 AND is_outlier=0 AND is_cancelled=0 AND delay_seconds IS NOT NULL AND ({pc}) GROUP BY stop_id HAVING n>=10",
#     conn,
# )
# print(f"\n=== AGGREGATION LEVELS ===")
# print(
...
# ---------------------------------------------------------------------------
# 6. Aggregation level comparison (RQ3)
# ---------------------------------------------------------------------------
stop_df = pd.read_sql(
